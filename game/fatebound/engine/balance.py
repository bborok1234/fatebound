...
MAX_ROUNDS = 36
TARGET_ROUNDS = (8, 16)
# 끝없는 강호(#9) — 마지막 존 보스 격파 후 같은 존을 깊이↑로 재도전. 깊이당 적 HP/공격 가산 스케일.
# HP는 더, 공격은 덜 올려 'DPS 레이스'(즉사 아닌 점진 한계). 깊이 사이 레벨업·드랍으로 플레이어도 성장.
# 라이트는 회귀 메타성장(insight)이 캐리. 고정 빌드 기준 깊이~8에서 도전 시작(실플레이는 더 깊이).
ENDLESS_HP_SCALE = 0.14
ENDLESS_ATK_SCALE = 0.04
